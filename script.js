...
// Valori: -2 (polo negativo del tema) .. +2 (polo positivo). Un valore per OGNI domanda (100 per partito).
// Segnaposto — fase 2: sostituire con posizioni reali, ognuna con { fonte, data }.
const PARTIES = Array.from({ length: 12 }, (_, i) => {
  const n = String(i + 1).padStart(2, "0");
  const seed = i * 7;
  const values = {};
  THEMES.forEach((theme, ti) => {
    values[theme.key] = theme.questions.map((_, qi) => (((seed + ti * 3 + qi * 5) % 5) - 2));
  });
  return {
    id: n,
    name: `Partito ${n}`,
...
// Coalizioni: raggruppamenti fittizi di partiti. Fase 2: alleanze reali con data di formalizzazione.
const COALITIONS = [
  { id: "c1", name: "Coalizione Fittizia 1", color: "#C1272D", partyIds: ["01", "02", "03", "04"] },
  { id: "c2", name: "Coalizione Fittizia 2", color: "#3E5C76", partyIds: ["05", "06", "07"] },
  { id: "c3", name: "Coalizione Fittizia 3", color: "#4E7A3A", partyIds: ["08", "09"] },
  { id: "c4", name: "Indipendenti", color: "#5B6478", partyIds: ["10", "11", "12"] },
];
...
// Legge elettorale: segnaposto strutturale. Fase 2: dati reali con fonte normativa.
const ELECTORAL_LAW = {
  nome: "Sistema segnaposto",
  tipo: "Misto proporzionale / maggioritario (esempio)",
  sogliaPartito: "3% (valore d'esempio)",
  sogliaCoalizione: "10% (valore d'esempio)",
  ripartizione: "Da definire in fase 2",
};
...
function computeMatch(party) {
  ...
  THEMES.forEach((theme) => {
    state.answers[theme.key].forEach((val, qi) => {
      if (val === undefined || val === 0) return; // "non mi esprimo" non pesa
      raw += party.values[theme.key][qi] * val;
      max += 2 * Math.abs(val);
    });
  });
...
function renderElectoralLaw() {
  const labels = {
    ...
    ripartizione: "Ripartizione seggi",
  };
...
function showProfile(id) {
  ...
  profileValues.innerHTML = THEMES.map((theme) => {
    const vals = p.values[theme.key]; // array di 10 valori -2..2
