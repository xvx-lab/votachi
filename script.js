...
// - "Economia" è popolato con 12 posizioni reali, parafrasate dalle fonti indicate in ognuna.
// - Gli altri 9 temi sono strutturalmente pronti ma vuoti ("in preparazione") — richiedono
//   la stessa ricerca verificata fatta per Economia, tema per tema.
...
function renderQuestion() {
  ...
  qCategoryEl.textContent = `${theme.label} · posizione ${state.posInOrder + 1}/${order.length} (partito anonimo)`;
  qTextEl.textContent = `Un partito sostiene: "${opinion.text}"`;
  answerButtons.forEach((b) => (b.disabled = false));
}
